...
void generate_shuffle_512_16_2(void) {
   std::ofstream shuffle_matrix("src/shuffle_matrix_512_16_2.c");

   // generate shuffle
   shuffle_matrix << "#include <immintrin.h>\n#include <tmmintrin.h>\n\n";
   for (uint64_t index = 0; index < SHUFFLE_SIZE; index++) {
      shuffle_matrix << "const __m512i RM" << std::to_string(index) << " = _mm512_set_epi16(\n";
   ...
   shuffle_matrix << '\n';
   shuffle_matrix << "__m512i SHUFFLE16_RIGHT [] = {";
   ...
   shuffle_matrix << "__m512i SHUFFLE16_LEFT [] = {";
   for (uint8_t i = 0; i < SHUFFLE_SIZE; i++) {
      shuffle_matrix << "RM" << std::to_string(i) << ", ";
   }
   for (uint8_t i = 0; i < SHUFFLE_SIZE; i++) {
      shuffle_matrix << "RM" << std::to_string(31);
      if (i < SHUFFLE_SIZE - 1)
         shuffle_matrix << ", ";
   }
